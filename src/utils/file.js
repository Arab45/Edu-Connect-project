...
const answerStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, 'public/files/imgs/diagrams');
  },
  filename: (req, file, cb) => {
    cb(
      null,
...
const diagramUpload = multer({ storage: answerStorage });
const answerImgUpload = diagramUpload.fields([
  { name: "diagram_image", maxCount: 1 },
  //   { name: "images", maxCount: 5 },
]);
